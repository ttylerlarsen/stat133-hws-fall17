...
miles <- c(1:100)
kms <-miles2kms(miles)
miles_kms <- data.frame(miles, kms)
```
...
gaussian <- function(x, m, s) {
  result <- (1 / (s*sqrt(2*3.1415))) * exp((-1/2)*((x-m)/s)*(x-m)/s)
  return(result)
}
```
...
```{r}
# your code
y_values2 <- gaussian(x_values, m = 0, s = 2)
...
x <- seq(-4, 4, length.out = 20)
y <- poly_a(x)
a_plot <- plot(x, y, type = 'l', lwd = 3, col = "#FB7215", las = 1)
abline(h = 0, v = 0, col = '#888888aa', lwd = 1.5)
...
binom_prob <- function(n, k, prob){
  y <- combinations(n, k) * prob^k * (1-prob)^(n-k)
  return(y)
}
binom_prob(n = 5, k = 4, prob = 0.4)
...
```{r}
k <- c(3, 4, 5)
prob1<- sum(binom_prob(5, k, .35))

prob2 <- binom_prob(10, 3, 1/6)
```
