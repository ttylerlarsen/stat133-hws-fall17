...
``` r
# total expenses
total <-  transpotation + groceries + gym + rent + other
```
...
expenses <- c(phone, transpotation, groceries, gym, rent, other)
expenses_sorted <- sort(expenses, TRUE)
```

Now, use the graphing function `barplot()` to produce a barchart of `expenses`:

``` r
barplot(expenses_sorted)
```
...
probability3 <- dbinom(k, n, p)

n <- 3
k <- 3
p <- 1/6

probability4 <- dbinom(k, n, p)
```
...
probability5 <- dbinom(k1, n, p) + dbinom(k2, n, p)
```
...
Work through the markdown tutorial: [www.markdown-tutorial.com](http://www.markdown-tutorial.com) Show in New WindowClear OutputExpand/Collapse Output \[1\] 0 \[1\] 7920 Show in New WindowClear OutputExpand/Collapse Output

Modify Chunk OptionsRun All Chunks AboveRun Current Chunk Show in New WindowClear OutputExpand/Collapse Output \[1\] 10 Show in New WindowClear OutputExpand/Collapse Output \[1\] 24 Modify Chunk OptionsRun All Chunks AboveRun Current ChunkModify Chunk OptionsRun All Chunks AboveRun Current Chunk Show in New WindowClear OutputExpand/Collapse Output Error in install.packages : Updating loaded packages Your turn: After lab discussion, find some time to go through this additional markdown tutorial [www.markdowntutorial.com/](http://www.markdowntutorial.com/)
